...
const lifecycleBundle = await build({ entryPoints: [fileURLToPath(new URL('../src/runtime/map-layer-stability.js', import.meta.url))],
  bundle: true, write: false, format: 'iife', globalName: 'stability', platform: 'browser' });
...
const server = createServer((req, res) => {
  ...
  res.end(req.url === '/maplibre.js' ? maplibre : req.url === '/overlay.js' ? bundle.outputFiles[0].text
    : req.url === '/stability.js' ? lifecycleBundle.outputFiles[0].text
    : '<html><body style="margin:0"><div id="map" style="width:1200px;height:800px"></div><script src="/maplibre.js"></script><script src="/overlay.js"></script><script src="/stability.js"></script></body></html>');
});
...
try {
  ...
  await page.evaluate(async (worldContextTilesUrl) => {
    ...
    stability.stabilizeMapLayerMoves(map);
  ...
  await page.evaluate(() => {
    map.remove();
    // Replay the native image.onload pattern after its map has been retired.
    if (!map.hasImage('late-icon')) map.addImage('late-icon', { width: 1, height: 1, data: new Uint8Array(4) });
    controller.dispose();
    controller.dispose();
