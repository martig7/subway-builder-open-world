...
test('map publication policy blocks raw or stale labels without affecting other worlds', () => {
  const definition = { map: { labelPolicy: 'japan-source-romaji-v2' } };
  assert.throws(() => assertMapLabelPolicy(definition, {}, 'JP_PREF_27'), /run the World's label publication stage/);
  assert.throws(() => assertMapLabelPolicy(definition, { labelPolicy: 'japan-source-romaji-v1' }, 'JP_PREF_27'), /does not match/);
  assert.doesNotThrow(() => assertMapLabelPolicy(definition, { labelPolicy: 'japan-source-romaji-v2' }, 'JP_PREF_27'));
