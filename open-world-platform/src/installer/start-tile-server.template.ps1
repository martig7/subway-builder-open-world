...
$pidPath = Join-Path $stateRoot "metro-maker4\{{NAMESPACE}}-pmtiles\server-$Port.pid"

function Test-TileArchiveRoot {
    param([string]$Candidate)
    return $Candidate -and (Test-Path -LiteralPath (Join-Path $Candidate '{{HEALTH_TILE_ID}}\tiles.pmtiles') -PathType Leaf)
}

function Resolve-TileArchiveRoot {
    if ($Root) {
        $explicitRoot = [System.IO.Path]::GetFullPath($Root)
        if (-not (Test-TileArchiveRoot $explicitRoot)) { throw "No Open World PMTiles archives found under $explicitRoot" }
        return $explicitRoot
    }
    $candidates = @(
    ...
    throw 'No Open World PMTiles archives found in the build, source package, or installed Subway Builder city data.'
}
...
if ($Stop) {
    if (-not $ExpectedInstallRoot) {
        throw 'ExpectedInstallRoot is required when stopping the Open World tile server.'
    }
    ...
    if ($remainingPids.Count) {
        throw "Verified Open World tile-server process did not stop: $($remainingPids -join ', ')"
    }
    ...
    Write-Host "Stopped $($existingPids.Count) verified Open World tile-server process(es) before installation."
    return
}
...
try {
    if (-not (Test-Path -LiteralPath $tilesRoot -PathType Container)) { throw "No generated Open World tile directory found: $tilesRoot" }
    if (-not (Test-Path -LiteralPath $serverScript -PathType Leaf)) { throw "Native PMTiles server script is missing: $serverScript" }
    if ($Background) {
        if (Test-TileServerHealth -HealthPort $Port) {
            Write-Host "Open World native PMTiles server is already running at http://127.0.0.1:$Port/"
            return
        }
        ...
        for ($attempt = 0; $attempt -lt 60; $attempt++) {
            ...
            if (Test-TileServerHealth -HealthPort $Port) {
                Write-Host "Open World native PMTiles server started at http://127.0.0.1:$Port/ (PID $($server.Id))."
                return
            }
    ...
    Write-Host "Starting the Open World native PMTiles server at http://127.0.0.1:$Port/"
    & $serverScript -Root $tilesRoot -BindHost '127.0.0.1' -Port $Port
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
} catch {
    Write-Host "Open World tile server stopped or failed to start:" -ForegroundColor Red
