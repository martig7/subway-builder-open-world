...
import { createViewportPresentationIndex } from './viewport-presentation-index.js';
...
const NATIVE_LAND_BACKING_ID = 'open-world-native-land-backing';
export const WORLD_BACKDROP_VERSION = 'native-footprint-world-backdrop-v1';
...
function maskMovementDeckLayers(
  ...
  if (dataEntry && virtualization) {
    ...
    const viewportEligible = retainedTrain && railRenderRevisions?.trainSimulationActive === true;
...
function hasVisibleTrainLayer(layers) {
  if (Array.isArray(layers)) return layers.some(hasVisibleTrainLayer);
  if (!layers || typeof layers !== 'object') return false;
  const layerId = layers.id ?? layers.props?.id;
  return /^trains(?:-under)?$/i.test(String(layerId))
    && (layers.props?.visible ?? layers.visible) !== false;
}

function shouldQueryActiveTrainViewport(patch) {
  const revisions = patch?.railRenderRevisionProvider?.();
  return revisions?.trainSimulationActive === true
    && revisions.trains != null
    && revisions.trainStyles != null
    && isDetailedMovementZoom(patch?.map?.getZoom?.())
    && hasVisibleTrainLayer(patch?.nativeLayers);
}

function applyMovementDeckCameraChange(deck, { refitViewport = false } = {}) {
  const patch = deck?.[MOVEMENT_DECK_GUARD_KEY];
  return applyMovementDeckVisibility(deck, {
    // Query only currently simulated, visible train layers. Stale indexes from
    // an earlier active interval must not cause work while cached simulation,
    // pause, overview visibility, or user layer hiding freezes presentation.
    force: shouldQueryActiveTrainViewport(patch),
    refitViewport,
  });
}
...
function ensureArtifacts(map, worldContextTilesUrl, worldVegetationData = null, nativeTile = null) {
  ...
  if (!map.getLayer?.(WORLD_OCEAN_LAYER_ID)) {
    map.addLayer?.({
      ...
      // Native land has a separate footprint backing above the world context.
      maxzoom: GEOGRAPHIC_CONTEXT_MAX_ZOOM,
      paint: { 'fill-color': theme.water, 'fill-opacity': 1 },
    }, firstNativeContentLayer);
  } else if (
    map.getLayer(WORLD_OCEAN_LAYER_ID)?.maxzoom !== GEOGRAPHIC_CONTEXT_MAX_ZOOM
    && typeof map.setLayerZoomRange === 'function'
  ) {
    try { map.setLayerZoomRange(WORLD_OCEAN_LAYER_ID, 0, GEOGRAPHIC_CONTEXT_MAX_ZOOM); } catch {}
  }
  ...
  const bounds = nativeTile?.nativeMapBounds ?? nativeTile?.bounds;
  const backingKey = JSON.stringify(bounds ?? null);
  const backingSource = safeMapSource(map, NATIVE_LAND_BACKING_ID);
  if (!backingSource || map.__openWorldNativeBackingKey !== backingKey) {
    const [w, s, e, n] = bounds ?? [];
    const data = [w, s, e, n].every(Number.isFinite) ? {
      type: 'FeatureCollection', features: [{ type: 'Feature', properties: {}, geometry: {
        type: 'Polygon', coordinates: [[[w,s],[e,s],[e,n],[w,n],[w,s]]],
      } }],
    } : EMPTY;
    if (!backingSource) map.addSource?.(NATIVE_LAND_BACKING_ID, { type: 'geojson', data });
    else backingSource.setData(data);
    map.__openWorldNativeBackingKey = backingKey;
  }
  if (!map.getLayer?.(NATIVE_LAND_BACKING_ID)) map.addLayer?.({
    id: NATIVE_LAND_BACKING_ID, type: 'fill', source: NATIVE_LAND_BACKING_ID,
    minzoom: STATION_MARKER_MIN_ZOOM, maxzoom: GEOGRAPHIC_CONTEXT_MAX_ZOOM,
    paint: { 'fill-color': theme.land, 'fill-opacity': 1, 'fill-antialias': false },
  }, firstNativeContentLayer);
  map.moveLayer?.(NATIVE_LAND_BACKING_ID, firstNativeContentLayer);
  map.__openWorldBackdropVersion = WORLD_BACKDROP_VERSION;
...
export class GeographicContextOverlayController {
  ...
  refreshContext() {
    return mapMovePerfMeasure('overlay.refresh.total', () => {
      ...
      // During handoff the runtime can already name the next tile while this
      // Map still displays the previous source. Prefer the bound source owner.
      const urls = safeMapSource(this.map, 'general-tiles')?.tiles ?? [];
      const nativeTile = this.tileCatalog.tiles.find(tile => urls.some(url => url.includes(`/${tile.id}/`)))
        ?? this.tileCatalog.tiles.find(tile => tile.id === this.activeTileId());
      mapMovePerfMeasure('overlay.ensure-artifacts', () => ensureArtifacts(this.map, this.worldContextTilesUrl, this.worldVegetationData, nativeTile));
      if (this.nativeParkSourceLayer === 'landuse') syncNativeParkLanduse(this.map);
