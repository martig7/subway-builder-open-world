...
$catalogPath = Join-Path $prototypeRoot 'generated\catalog\tokyo-kanagawa-tile-catalog.json'
...
if (-not (Test-Path -LiteralPath $catalogPath -PathType Leaf)) {
    throw "Missing Tokyo-Kanagawa tile catalog: $catalogPath"
}
...
if ($unknownTileIds.Count -gt 0) {
    throw "Unknown Tokyo-Kanagawa tile IDs: $($unknownTileIds -join ', ')"
}
...
function ContainerPath([string]$HostPath) {
    ...
    if (-not $resolvedHostPath.StartsWith($rootPrefix, [System.StringComparison]::OrdinalIgnoreCase)) {
        throw "Path is outside the Tokyo-Kanagawa prototype root: $resolvedHostPath"
    }
...
Write-Host 'Unified z0-z9 world / z10-z15 Tokyo-Kanagawa PMTiles archives are ready. Original city-only archives are retained as tiles.city-only.pmtiles.'
