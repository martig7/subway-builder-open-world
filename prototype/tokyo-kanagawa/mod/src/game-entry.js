...
const PENDING_NAVIGATION_KEY = 'tokyo-kanagawa:pending-navigation';
const PENDING_PERFORMANCE_KEY = 'tokyo-kanagawa:pending-performance';
const DIAGNOSTICS_KEY = 'diagnostics:tile-transitions';
const CURRENT_CANONICAL_WORLD_ID = 'tokyo-kanagawa-world';
...
(function installTokyoKanagawaWorld() {
  ...
  api.ui?.unregisterComponent?.('top-bar', 'tokyo-kanagawa-world-saves');
  api.ui?.unregisterComponent?.('main-menu', 'tokyo-kanagawa-world-saves-home');
  installTransientLayerOrderConsoleFilter();
  stabilizeMapLayerMoves(api.utils?.getMap?.());
  relaxMapZoomLimits(api.utils?.getMap?.(), { sourceMinZoom: tileCatalog.basemapMinZoom });
  const generationKey = '__tokyoKanagawaGeneration__';
  ...
  const nativeSaveLifecycle = createNativeSaveLifecycle({
    ...
    storageKey: 'tokyo-kanagawa:pending-native-save-echo',
    trace: loadTrace,
  });
  ...
  const identities = new WorldIdentityResolver({
    storage,
    fallbackWorldId: 'tokyo-kanagawa',
    canonicalWorldId: CURRENT_CANONICAL_WORLD_ID,
  });
  const tileBase = globalThis.TOKYO_KANAGAWA_TILE_BASE ?? 'http://127.0.0.1:8800';
  ...
  const diagnostics = globalThis.__tokyoKanagawaDiagnostics__ = {
    generation,
  ...
  const runtime = new WorldTileRuntime({
    ...
    initialWorld: { activeTileId: 'JP_TOKYO_MAINLAND', wallet: 1_000_000, cohorts: [] },
    telemetry: (event) => {
  ...
  async function start(loadedCityCode, saveName = loadedSaveName, { replaceWorld = false } = {}) {
  ...
  async function handleGameLoaded(saveName) {
    ...
    if (loadKind === 'save-echo' || loadKind === 'internal-runtime') {
      ...
      if (!started && registration.cities.includes(loadedCityCode)) {
        ...
        return start(loadedCityCode, null);
      }
