...
const packagesPath = path.resolve(root, '..', 'generated', 'pilot', 'tiles');
...
async function modDirectoryName() {
  ...
  if (!directoryName || !/^[a-zA-Z0-9_-]+$/.test(directoryName)) throw new Error(`Unsafe mod id: ${id || '(empty)'}`);
...
const tileServer = await ensureTileServerReady({
  starterPath: path.resolve(root, '..', 'tools', 'start-canary-server.ps1'),
});
