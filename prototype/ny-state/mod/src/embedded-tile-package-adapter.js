...
export class EmbeddedTilePackageAdapter {
  ...
  async prepare(tileId) {
    if (!this.catalogTileIds.includes(tileId)) throw new Error(`Unknown New York pilot tile: ${tileId}`);
  ...
  async loadCommuteCatalog(tileId) {
    ...
    if (!this.embeddedData?.commuteCatalog) throw new Error('Embedded New York commute catalog is unavailable');
  ...
  #assertTile(tileId) {
    if (!this.catalogTileIds.includes(tileId)) throw new Error(`Unknown New York canary tile: ${tileId}`);
  }
  async #decodeCrossDemand() {
    ...
    if (!encoded) throw new Error('Embedded New York cross-demand data is unavailable');
