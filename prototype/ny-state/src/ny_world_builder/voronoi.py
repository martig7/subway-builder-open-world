...
def cluster_demand_sites(
    ...
    """Build an irregular weighted Voronoi partition without constructing polygons.

    A deterministic random priority chooses a maximal set of source-point seeds.
    A sparse square hash only accelerates radius queries; it does not determine
    seed positions or output coordinates. Every source is then assigned to its
    nearest seed and each published site is the cell's weighted centroid.
    """
    if radius <= 0:
    ...
    for index in populated:
        ...
        site_id = f"demand-site-{_stable_id('ny-voronoi-v2', tile_id, *cell_members)}"
        sites[site_id] = {
            "id": site_id,
            "tileId": tile_id,
...
def _merge_undersized_cells(cells: list[dict[str, Any]], minimum_size: int, maximum_size: int) -> None:
    """Greedily coarsen adjacent 4D Voronoi cells until every cell is viable."""
    while True:
        ...
        for source_index, neighbors in ordered:
            ...
            for local_index in np.atleast_1d(neighbors):
                ...
                choices.append((category, distance, _stable_priority(target["anchor"]), target_index))
            if not choices:
                continue
        ...
        if merged_this_round == 0:
            # All cached neighbors were consumed earlier in the round. Rebuilding
            # the tree exposes the remaining active neighbors on the next pass.
            continue
...
def pack_voronoi_cohorts(
    ...
    *,
    ...
    """Aggregate OD flows with a weighted 4D Voronoi partition.

    Undersized OD pairs are sampled proportional to their mass to obtain an
    irregular seed field with about ``total_mass / target_size`` cells. A second
    streaming pass assigns every pair to its nearest seed in (home, work) space.
    """
    ...
    if seed_rows:
        seed_positions = np.asarray(
            [(*coordinates[home_site], *coordinates[work_site]) for home_site, work_site, _ in seed_rows],
            dtype=np.float64,
        )
        ...
        def flush() -> None:
            ...
            for dimension in range(4):
                coordinate_sums[:, dimension] += np.bincount(
                    assigned,
                    weights=positions[:, dimension] * masses,
                    minlength=len(seed_rows),
                )
        ...
        for index, mass in enumerate(mass_sums.tolist()):
            ...
            cells.append({
                "position": tuple((coordinate_sums[index] / mass).tolist()),
                "mass": mass,
                "anchor": f"{home_site}\x1f{work_site}",
                "active": True,
            })

    for home_site, work_site, mass in direct_rows:
        cells.append({
            "position": (*coordinates[home_site], *coordinates[work_site]),
            "mass": mass,
            "anchor": f"{home_site}\x1f{work_site}",
            "active": True,
        })
