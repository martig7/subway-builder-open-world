...
const WORKER_NAME = 'nec-generated-road-route-worker';
const NATIVE_PREFIX = 'nec-native-pop-';
const CROSS_PREFIX = 'nec-cross-pop-';
 ...
 */
export function createNecRoutePaths({
  tilePackages,
  tileCatalog,
  getNativeDemand,
  workerSource = null,
  ...
  const owns = (city, popId) => tileIds.has(city)
    && (String(popId).startsWith(NATIVE_PREFIX) || String(popId).startsWith(CROSS_PREFIX));

  async function endpoints(city, popId) {
    if (String(popId).startsWith(NATIVE_PREFIX)) return nativeEndpoints(getNativeDemand?.(), popId, city);
    if (!crossIndex) {
