...
import { projectOffTileNativeDemandTransferInput } from '../../../kc-two-tile/mod/src/off-tile-native-demand.js';

const DEMAND_JSON_DECODER_VERSION = 1;
const DEMAND_JSON_DECODER_STATE = '__necCorridorDemandJsonDecoder';
const NATIVE_DEMAND_EVALUATOR_VERSION = 1;
const NATIVE_DEMAND_EVALUATOR_STATE = '__necCorridorNativeDemandEvaluator';
const NATIVE_DEMAND_EVALUATOR_WORKER_NAME = 'nec-native-demand-worker-evaluator-v1';
...
export function createOffMainThreadNativeDemandEvaluator({
  ...
  createObjectURL = globalThis.URL?.createObjectURL?.bind(globalThis.URL),
  revokeObjectURL = globalThis.URL?.revokeObjectURL?.bind(globalThis.URL),
  workerSource = typeof __NEC_NATIVE_DEMAND_EVALUATOR_WORKER_SOURCE__ === 'string'
    ? __NEC_NATIVE_DEMAND_EVALUATOR_WORKER_SOURCE__
    : null,
} = {}) {
  let worker = null;
  let workerUrl = null;
...
function sharedNativeDemandEvaluator() {
  const existing = globalThis[NATIVE_DEMAND_EVALUATOR_STATE];
  if (existing?.version === NATIVE_DEMAND_EVALUATOR_VERSION && existing.evaluator) {
...
export class EmbeddedTilePackageAdapter {
  constructor(tileIds, embeddedData, {
    ...
    this.evaluateNativeDemandBytes = evaluateNativeDemandBytes
      ?? sharedNativeDemandEvaluator().evaluate;
  ...
  canSkipNativeDemandForUnservedTile(tileId) {
    ...
    // Every installed NEC tile has a native demand package. A tile with no
  ...
  async loadRoadBytes(tileId) {
    this.#assertTile(tileId);
    if (typeof this.fetchData !== 'function') throw new Error('NEC road data fetch is unavailable');
