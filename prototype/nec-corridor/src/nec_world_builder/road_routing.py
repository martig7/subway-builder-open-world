...
def enrich_nec_driving(
    ...
    *,
    ...
    partition_cache_path = stage / "reports" / "nec-cross-route-models.json"
    ...
    commutes["buildHash"] = f"nec-road-v1-{_stable_id(*commute_parts)}"
    ...
    demand_report_path = demand / "reports" / "nec-demand.json"
    ...
    demand_report["roadRouting"] = {
        "report": "nec-road-routing.json",
        "graphVersion": GRAPH_VERSION,
        "completed": True,
    }
    _write_json(stage / "reports" / "nec-demand.json", demand_report)

    routing_report = {
        "schemaVersion": 1,
        "status": "complete",
        "kind": "generated-road driving-time splice",
        "consumerManifestId": "local.nec-corridor-open-world",
        "graph": graph_report,
    ...
    _write_json(stage / "reports" / "nec-road-routing.json", routing_report)
